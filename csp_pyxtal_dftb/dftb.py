...
class Dftb(FileIOCalculator):
    ...
    def read_eigenvalues(self):
        ...
        print(
            'ncol:', ncol, 'nrow:', nrow,
            'index_eig_begin:', index_eig_begin, 
            'index_eig_end:' ,index_eig_end
        )

        #eig = np.loadtxt(self.lines[index_eig_begin:index_eig_end]).flatten()
        from io import StringIO   # StringIO behaves like a file object
        s = ''.join(self.lines[index_eig_begin:index_eig_end])
        print('eigenvals in DFTB+:\n', s)
        c = StringIO(s.replace('\n', ''))
        eig = np.loadtxt(c).flatten()
        eig *= Hartree
        N = nkpt * nband
